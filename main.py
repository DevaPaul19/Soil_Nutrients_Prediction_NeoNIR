...
def preprocess_data(new_data, scaler_path, encoder_path, other):
...
def predict_with_model(model_path, new_data_reshaped, soil_nut):
    # Load the model
    model_lstm = tf.keras.models.load_model(model_path, custom_objects={'rmse_loss': rmse_loss})

    # Predict using the trained model
    predictions = model_lstm.predict(new_data_reshaped)
...
def main():
    ...
    if uploaded_file is not None:
        ...
        # Preprocess data
        new_data_reshaped = preprocess_data(new_data, scaler_path, encoder_path, other)

        # Load and predict with model
        model_path = 'neo_model_lstm_wm.keras'  # Change this to the actual path
        model_lstm = tf.keras.models.load_model(model_path, custom_objects={'rmse_loss': rmse_loss})

        # Predict using the trained model
        predictions = model_lstm.predict(new_data_reshaped)
        predictions_df = pd.DataFrame(predictions, columns=soil_nut)
        predictions_df.style.applymap(lambda x: 'color: black')
        # Display predictions
